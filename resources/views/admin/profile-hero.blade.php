...
@section('title', 'Kelola Profile Hero - Admin Panel')
...
@section('content')
    ...
    <div class="mb-6">
        <h1 class="text-2xl font-bold text-[#183D57]">Kelola Profile Hero</h1>
        <p class="text-gray-500">Mengatur tampilan hero section di halaman Profile</p>
    </div>

    <div class="bg-white rounded-2xl shadow-sm overflow-hidden">
        <div class="bg-gradient-to-r from-[#183D57] to-[#2a5a7a] px-6 py-4">
            <h3 class="text-white font-bold text-lg">Edit Profile Hero</h3>
        </div>

        <form action="{{ route('admin.profile-hero.update', $hero->id) }}" method="POST" enctype="multipart/form-data"
            class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">Text kecil di atas judul (contoh: Tentang Kami)</p>
                    </div>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="button_primary_link"
                            ...
                            class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:border-[#8AD337] focus:ring-2 focus:ring-[#8AD337]/20">
                        <p class="text-xs text-gray-400 mt-1">Contoh: #sejarah, /profile</p>
                    </div>
                    ...
                    <div>
                        ...
                        @php
                            $imagePath = public_path('uploads/profile-hero/' . $hero->hero_image);
                        @endphp

                        <div id="profileHeroImagePreviewWrapper"
                            class="{{ $hero->hero_image && file_exists($imagePath) ? '' : 'hidden' }} mb-3 p-2 bg-gray-100 rounded-lg">
                            <p id="profileHeroImagePreviewLabel" class="text-xs text-gray-500 mb-2">Gambar saat ini:</p>
                            <img id="profileHeroImagePreview"
                                src="{{ $hero->hero_image && file_exists($imagePath) ? asset('uploads/profile-hero/' . $hero->hero_image) : '' }}"
                                alt="Preview Gambar Profile Hero" class="h-32 w-auto rounded-lg border shadow-sm">
                            <p id="profileHeroImagePreviewName" class="text-xs text-gray-400 mt-1">{{ $hero->hero_image }}</p>
                        </div>

                        <input id="profileHeroImageInput" type="file" name="hero_image" accept="image/*"
                            class="w-full px-4 py-2 border border-gray-300 rounded-xl focus:outline-none focus:border-[#8AD337] focus:ring-2 focus:ring-[#8AD337]/20">
    ...
    <div class="mt-8">
        ...
        <div class="bg-gradient-to-r from-gray-100 to-gray-200 rounded-2xl p-6 border-2 border-dashed border-gray-300">
            <div class="text-center">
                ...
                @if ($hero->hero_image && file_exists(public_path('uploads/profile-hero/' . $hero->hero_image)))
                    <div class="my-4">
                        <img src="{{ asset('uploads/profile-hero/' . $hero->hero_image) }}" alt="Preview"
                            class="h-40 w-auto rounded-lg shadow-md mx-auto object-cover">
                    </div>
                @else
                    <div class="my-4 w-full h-32 bg-gray-200 rounded-lg flex items-center justify-center">
                        <i class="fas fa-image text-3xl text-gray-400"></i>
                    </div>
                @endif
                <p class="text-gray-500 text-sm mt-2 max-w-2xl mx-auto">{{ Str::limit($hero->description, 150) }}</p>
    ...
    <script>
        const profileHeroImageInput = document.getElementById('profileHeroImageInput');
        const profileHeroImagePreviewWrapper = document.getElementById('profileHeroImagePreviewWrapper');
        const profileHeroImagePreview = document.getElementById('profileHeroImagePreview');
        const profileHeroImagePreviewLabel = document.getElementById('profileHeroImagePreviewLabel');
        const profileHeroImagePreviewName = document.getElementById('profileHeroImagePreviewName');
        let profileHeroImagePreviewUrl = null;

        profileHeroImageInput?.addEventListener('change', (event) => {
            ...
            if (profileHeroImagePreviewUrl) {
                URL.revokeObjectURL(profileHeroImagePreviewUrl);
            }

            profileHeroImagePreviewUrl = URL.createObjectURL(file);
            profileHeroImagePreview.src = profileHeroImagePreviewUrl;
            profileHeroImagePreviewLabel.textContent = 'Preview gambar baru:';
            profileHeroImagePreviewName.textContent = file.name;
            profileHeroImagePreviewWrapper.classList.remove('hidden');
        });
    </script>
@endsection
